...
#ifdef CONFIG_NDCKPT
	pgtable_t token = ndckpt_is_pmd_points_nvdimm_page(*pmd) ?
				  NULL :
				  pmd_pgtable(*pmd);
#else
...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	 ...
	 */
	vmf->pte = pte_offset_map_lock_wrapper(mm, vmf->pmd, vmf->address,
					       &vmf->ptl);
...
vm_fault_t finish_mkwrite_fault(struct vm_fault *vmf)
{
	...
	vmf->pte = pte_offset_map_lock_wrapper(vmf->vma->vm_mm, vmf->pmd,
					       vmf->address, &vmf->ptl);
	/*
...
static vm_fault_t do_wp_page(struct vm_fault *vmf) __releases(vmf->ptl)
{
	...
	if (PageAnon(vmf->page)) {
		...
		if (!trylock_page(vmf->page)) {
			...
			pte_unmap_unlock(vmf->pte, vmf->ptl);
			lock_page(vmf->page);
			vmf->pte = pte_offset_map_lock_wrapper(
				vma->vm_mm, vmf->pmd, vmf->address, &vmf->ptl);
			if (!pte_same(*vmf->pte, vmf->orig_pte)) {
				unlock_page(vmf->page);
				pte_unmap_unlock(vmf->pte, vmf->ptl);
				put_page(vmf->page);
...
vm_fault_t do_swap_page(struct vm_fault *vmf)
{
	...
	if (unlikely(non_swap_entry(entry))) {
		if (is_migration_entry(entry)) {
			 ...
			 */
			ret = device_private_entry_fault(
				vma, vmf->address, entry, vmf->flags, vmf->pmd);
...
static vm_fault_t do_anonymous_page(struct vm_fault *vmf)
{
	 ...
	 */
#ifdef CONFIG_NDCKPT
	if (ndckpt_pte_alloc(vma->vm_mm, vmf->pmd, vmf->vma, vmf->address))
		return VM_FAULT_OOM;
#else
...
static vm_fault_t pte_alloc_one_map(struct vm_fault *vmf)
{
	...
	if (vmf->prealloc_pte) {
		...
		ndckpt_pmd_populate(vma->vm_mm, vmf->pmd,
				    page_to_virt(vmf->prealloc_pte));
...
static inline void validate_pgtable_for_ndckpt(struct vm_fault *vmf, int case_id)
{
	...
	pgd = vmf->vma->vm_mm->pgd;
	if (ndckpt_is_virt_addr_in_nvdimm(pgd) && vmf->pud &&
	    ndckpt_is_phys_addr_in_nvdimm(vmf->pud->pud & PTE_PFN_MASK) &&
	    vmf->pmd &&
	    ndckpt_is_phys_addr_in_nvdimm(vmf->pmd->pmd & PTE_PFN_MASK) &&
	    vmf->pte &&
	    ndckpt_is_phys_addr_in_nvdimm(vmf->pte->pte & PTE_PFN_MASK)) {
		ndckpt_invlpg((void *)vmf->address);
		return;
	}
	printk("invalid pgtable structure for pmem page. case=%d\n", case_id);
	pr_ndckpt_pgtable_range(pgd, vmf->address, vmf->address + 1);
	BUG();
}
...
static vm_fault_t handle_pte_fault_ndckpt(struct vm_fault *vmf)
{
  ...
  if(vma_is_anonymous(vmf->vma)) {
	...
	if (!vmf->pte) {
		if (!vma_is_anonymous(vmf->vma)) {
			pr_ndckpt_fault(
				"fault on non-anonymous page 0x%016lX\n",
				vmf->address);
			fault_code = handle_pte_fault_body(vmf);
			if (!vmf->pte || !pte_present(*vmf->pte))
				return fault_code;
			// page is mapped on dram by handle_pte_fault_body. replace it with nvdimm.
			ndckpt_replace_page_with_nvdimm_page(vmf->pte,
							     vmf->address);
			validate_pgtable_for_ndckpt(vmf, 1);
			return fault_code;
		}
		...
		BUG_ON(!vmf->pte);
		if (!ndckpt_is_phys_addr_in_nvdimm(vmf->pmd->pmd &
						   PTE_PFN_MASK)) {
			replace_pt_with_nvdimm_page(vmf->pmd);
			ndckpt_invlpg((void *)vmf->address);
		}
		BUG_ON(ndckpt_is_phys_addr_in_nvdimm(vmf->pte->pte &
						     PTE_PFN_MASK));
		ndckpt_replace_page_with_nvdimm_page(vmf->pte, vmf->address);
		pr_ndckpt_fault(
			"fault on anonymous page 0x%016lX. pte becomes 0x%016llX\n",
			vmf->address, (uint64_t)vmf->pte->pte);
		validate_pgtable_for_ndckpt(vmf, 2);
		return 0;
	}
	...
	if (!vma_is_anonymous(vmf->vma) && !pte_write(*vmf->pte)) {
		...
		if (ndckpt_is_pte_points_nvdimm_page(*vmf->pte)) {
			pte_mkwrite(*vmf->pte);
			validate_pgtable_for_ndckpt(vmf, 3);
			return 0;
		}
		fault_code = handle_pte_fault_body(vmf);
		...
		validate_pgtable_for_ndckpt(vmf, 4);
		return fault_code;
	}
	pr_ndckpt_fault(
		"pte fault in target vma (existed) @ 0x%016lX flags=0x%08X\n",
		vmf->address, vmf->flags);
	BUG_ON(!ndckpt_is_phys_addr_in_nvdimm(vmf->pud->pud & PTE_PFN_MASK));
	BUG_ON(!ndckpt_is_phys_addr_in_nvdimm(vmf->pmd->pmd & PTE_PFN_MASK));
	BUG_ON(!ndckpt_is_phys_addr_in_nvdimm(vmf->pte->pte & PTE_PFN_MASK));
	pte = pte_mkwrite(pte_mkdirty(*vmf->pte));
	/* No need to invalidate - already invalidated by fault */
	*vmf->pte = pte;
	validate_pgtable_for_ndckpt(vmf, 5);
	return 0;
}
#endif
